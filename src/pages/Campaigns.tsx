...
import { Plus, Play, Pause, Mail, Clock, ArrowRight, Users, Sparkles, CheckCircle2, Trash2 } from "lucide-react";
...
const Campaigns = () => {
  const { leads, campaigns, addCampaign, launchCampaign } = useSales();
  ...
  const handleCreate = () => {
    const count = leads.length || 5;
    addCampaign(campaignName, count);
    setNewCampaignId(Date.now());
    setActiveStep(4);
    toast.success(`Campaign "${campaignName}" created!`);
  };

  const handleLaunch = () => {
    // Find the most recent campaign to launch
    const target = campaigns[campaigns.length - 1];
    if (target) {
      launchCampaign(target.id);
    }
    toast.success("Campaign launched! Emails are being sent to all leads.");
    setShowCreate(false);
    setActiveStep(0);
  };
    ...
    <>
      ...
      <main className="flex-1 overflow-y-auto p-6 space-y-6">
        <div className="flex items-center gap-3">
          <button
            onClick={() => { setShowCreate(!showCreate); setActiveStep(0); }}
          ...
          <div className="glass-card p-6 animate-fade-in">
              ...
              {activeStep === 0 && (
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mb-4">
                    {leads.length > 0
                      ? `You have ${leads.length} leads available.`
                      : "No leads imported yet — go to Leads tab to import some first, or continue with demo data."}
                  </p>
                  <div className="flex gap-3">
                    <button className="px-4 py-2 rounded-xl border border-primary bg-primary/10 text-sm text-primary font-medium">
                      All Leads ({leads.length || 5})
                    </button>
                  </div>
                </div>
              )}
              ...
              {activeStep === 4 && (
                <div className="flex flex-col items-center justify-center py-8">
                  ...
                  <p className="text-sm text-muted-foreground mb-6">
                    Your campaign "{campaignName}" will reach {leads.length || 5} leads with {sequenceSteps.length} step{sequenceSteps.length > 1 ? "s" : ""}.
                  </p>
                  <button
            ...
            <div className="flex justify-between mt-4">
              ...
              {activeStep < 4 && (
                ...
                >
                  {activeStep === 3 ? "Create & Review" : "Next Step"}
                </button>
              )}
            </div>
          </div>
